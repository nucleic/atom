...
PyMethodDef Member_methods[] = {
	...
	{ "do_validate",
		...
		METH_VARARGS,
		"do_validate(atom, name, value) run the validate handler for the member" },
	{ "do_post_validate",
		( PyCFunction )Member_do_post_validate,
		METH_VARARGS,
		"do_post_validate(atom, name, value) run the post validate handler for the member" },
	{ "do_post_setattr",
		( PyCFunction )Member_do_post_setattr,
		METH_VARARGS,
		"do_post_setattr(atom, name, old, new) run the post setattr handler for the member" },
	{ "validation_error",
		( PyCFunction )Member_validation_error,
		METH_VARARGS,
		"validation_error(atom, name, value) raise a ValidationError for the member" },
	{ 0 } // sentinel
};
