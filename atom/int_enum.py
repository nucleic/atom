...
def int_enum_flags_unpickler(enum_class, value):
    return int.__new__(enum_class.Flags, value)


def int_enum_flags_pickler(flags):
    enum_class = type(flags).__enum_class__
    return (int_enum_flags_unpickler, (enum_class, int(flags)))
...
class IntEnumMeta(type):
    ...
    def __call__(cls, which):
        ...
        if isinstance(which, int):
            ...
            msg = "enum specifier must be an int or string, "
...
class IntEnumFlags(int):
    """ An int subclass for create IntEnum flags.

    This class should not be instantiated directly by user code.
    ...
    __mul__ = invalid_op('*')
    __div__ = invalid_op('/')
    __truediv__ = invalid_op('/')
    __floordiv__ = invalid_op('//')
    __mod__ = invalid_op('%')
    __divmod__ = invalid_op('divmod')
    __pow__ = invalid_op('**')
    __lshift__ = invalid_op('<<')
    __rshift__ = invalid_op('>>')
    __radd__ = invalid_op('+')
    __rsub__ = invalid_op('-')
    __rmul__ = invalid_op('*')
    __rdiv__ = invalid_op('/')
    __rtruediv__ = invalid_op('/')
    __rfloordiv__ = invalid_op('//')
    __rmod__ = invalid_op('%')
    __rdivmod__ = invalid_op('divmod')
    __rpow__ = invalid_op('**')
    __rlshift__ = invalid_op('<<')
    __rrshift__ = invalid_op('>>')
    __neg__ = invalid_op('-')
    __pos__ = invalid_op('+')
    __abs__ = invalid_op('abs')


class IntEnum(int):
    """ An int subclass for declaring enum types.

    """
    __metaclass__ = IntEnumMeta

    __slots__ = '__enum_name__'
