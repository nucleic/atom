...
static int CAtom_init( PyObject* self, PyObject* args, PyObject* kwargs )
{
...
static void CAtom_clear( CAtom* self )
{
...
static int CAtom_traverse( CAtom* self, visitproc visit, void* arg )
{
    uint32_t count = ClassMap_MemberCount( self->class_map );
    for( uint32_t i = 0; i < count; ++i )
    {
...
static void CAtom_dealloc( CAtom* self )
{
...
static PyObject* CAtom_getattro( CAtom* self, PyStringObject* name )
{
    uint32_t index;
    Member* member = 0;
    ...
    if( member )
    {
        return Member_GetAttr( member, self, name, self->slots + index );
    }
    return PyObject_GenericGetAttr( ( PyObject* )self, ( PyObject* )name );
}


static int CAtom_setattro( CAtom* self, PyStringObject* name, PyObject* value )
{
    uint32_t index;
    Member* member = 0;
    ClassMap_LookupMember( self->class_map, name, &member, &index );
    if( member )
    {
        return Member_SetAttr( member, self, name, value, self->slots + index );
    }
...
static PyObject* CAtom_sizeof( CAtom* self, PyObject* args )
{
